                    ...
                    >
                        <div className="flex items-start justify-between mb-3">
                            <div className="space-y-1">
                                <div className="h-5 w-20 bg-[#2d303a]/50 rounded animate-pulse" />
                                <div className="h-3 w-16 bg-[#2d303a]/50 rounded animate-pulse" />
                            </div>
                            <div className="h-6 w-24 bg-[#2d303a]/50 rounded-lg animate-pulse" />
                        </div>
                        <div className="flex justify-center mb-3">
                            <div className="h-16 w-16 rounded-full bg-[#2d303a]/50 animate-pulse" />
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                            {[1, 2, 3, 4].map((j) => (
                                ...
                                >
                                    <div className="h-3 w-12 bg-[#2d303a]/50 rounded animate-pulse mb-1" />
                                    <div className="h-5 w-16 bg-[#2d303a]/50 rounded animate-pulse" />
                                </div>
                            ))}
                        </div>
                    </div>
    ...
    return (
        <div className="space-y-3">
            ...
            {stocksWithValuation.map((stock) => {
                ...
                const peScore = Math.max(
                    0,
                    Math.min(100, 100 - ((stock.peRatio - 15) / 25) * 100)
                );
                const roeScore = (stock.roe / 30) * 100;
                const debtScore = Math.max(
                    0,
                    100 - (stock.debtToEquity / 2) * 100
                );
                ...
                return (
                    ...
                    >
                        <div className="mb-2 flex items-start justify-between">
                            <div>
                                <h3 className="font-bold text-base text-[#e8eaed]">
                                    {stock.symbol}
                                </h3>
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-2">
                            <div className="rounded-lg bg-black/30 p-2 space-y-1">
                                <div className="flex items-center justify-between mb-0.5">
                                    ...
                                    <p className="text-sm font-bold text-[#e8eaed]">
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                            <div className="rounded bg-black/30 p-1.5 text-center">
                                ...
                                <p className="text-xs font-bold text-[#e8eaed]">
                                    ₹{stock.eps.toFixed(2)}
                                </p>
                            </div>
                            <div className="rounded bg-black/30 p-1.5 text-center">
                ...
                <div className="grid grid-cols-3 gap-2">
                    <div className="text-center">
                        <p className="text-xl font-bold text-[#3dd68c]">
                            {
                                stocksWithValuation.filter(
                                    (s) => s.rating === "Undervalued"
                                ).length
                            }
                        </p>
                        <p className="text-[10px] text-[#8b8f9a]">
                    ...
                    <div className="text-center border-x border-[#2d303a]/40">
                        <p className="text-xl font-bold text-[#e8eaed]">
                            {(
                                stocksWithValuation.reduce(
                                    (sum, s) => sum + s.peRatio,
                                    0
                                ) / stocksWithValuation.length
                            ).toFixed(1)}
                        </p>
